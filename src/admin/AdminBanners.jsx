...
import { FaTrash, FaEdit, FaEye, FaToggleOn, FaToggleOff, FaCloudUploadAlt } from "react-icons/fa"; // ✅ FaCloudUploadAlt add cheythu
import { getAllAdsAPI, deleteAdAPI, updateAdAPI } from "../apis/Api";
import "../styles/adminBanners.css";

const API_URL = "http://10.132.253.42:8000"; // ✅ Base URL
...
export default function AdminBanners() {
  ...
  const handleAdSubmit = async (e) => {
    ...
    try {
      ...
      if (editId) {
        ...
        const response = await fetch(`${API_URL}/api/auth/admin/ads/`, { // ✅ Fixed
          method: "POST",
          headers: { "Authorization": `Bearer ${token}` },
          body: formData,
        });
  ...
  const handleToggleActive = async (banner) => {
    try {
      const token = localStorage.getItem("access");
      const response = await fetch(`${API_URL}/api/auth/admin/ads/${banner.id}/`, { // ✅ Fixed
        method: "PATCH",
        headers: {
          "Authorization": `Bearer ${token}`,
          "Content-Type": "application/json",
    ...
    <div className="admin-content">
      ...
      <div className="panel" style={{ padding: "30px", marginTop: "20px", backgroundColor: "#fff", borderRadius: "12px", boxShadow: "0 4px 12px rgba(0,0,0,0.05)" }}>
        ...
        <form onSubmit={handleAdSubmit} style={{ display: "flex", flexDirection: "column", gap: "20px", maxWidth: "600px" }}>
          ...
          <div style={{ display: "flex", gap: "12px", marginTop: "10px" }}>
            <button type="submit" disabled={uploading} style={{ height: "48px", padding: "0 24px", background: "linear-gradient(135deg, #e91662, #ff4b91)", color: "white", border: "none", borderRadius: "8px", cursor: "pointer", fontWeight: "900", fontSize: "14px" }}>
              {uploading ? "Processing..." : editId ? "Update Ad" : "+ Publish Banner"}
            </button>
            {editId && (
              <button type="button" onClick={resetForm} style={{ height: "48px", padding: "0 24px", background: "#64748b", color: "white", border: "none", borderRadius: "8px", cursor: "pointer", fontWeight: "900", fontSize: "14px" }}>
                Cancel
              </button>
            )}
          </div>
                ...
                {banners.map((banner) => (
                  <tr key={banner.id}>
                    <td>
                      {banner.file_type === 'image' ? (
                        <img
                          src={`${API_URL}${banner.file_url}`} // ✅ Fixed
                          alt={banner.title}
                          style={{ width: "80px", height: "50px", objectFit: "cover", borderRadius: "4px" }}
                          onError={(e) => {e.target.src = '/placeholder.png'}}
                        />
                    ...
                    <td>{banner.title}</td>
                    <td><span className={`status ${banner.file_type}`}>{banner.file_type}</span></td>
                    <td>{banner.link_url ? (<a href={banner.link_url} target="_blank" rel="noopener noreferrer"><FaEye /> View</a>) : "—"}</td>
                    <td>
                      <button onClick={() => handleToggleActive(banner)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: "20px" }}>
                    ...
                    <td>
                      <div className="user-action-icons">
                        <button title="Edit" onClick={() => handleEdit(banner)}><FaEdit /></button>
                        <button title="Delete" className="delete" onClick={() => handleDelete(banner.id)}><FaTrash /></button>
                      </div>
                    </td>
                  </tr>
                ))}

...
export default function ChatPage() {
  ...
  // 🛠️ ഫെയിൽ സേഫ് ഇമേജ് യുആർഎൽ സിസ്റ്റം (ബാഹ്യ പ്ലേസ്ഹോൾഡർ ഒഴിവാക്കി)
  const getImageUrl = (path) => {
    if (!path) return null;
    return path.startsWith("http") ? path : `${API_BASE_URL}${path}`;
  };

  // ഇമേജ് ലോഡിങ് പരാജയപ്പെട്ടാൽ കാണിക്കാനുള്ള തദ്ദേശീയ ഫംഗ്ഷൻ
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const payload = { room_id: selectedRoom.id, text: newMessage };
  ...
  const handleRequestAction = async (action) => {
    try {
      ...
      const backendError = err.response?.data?.error || err.response?.data?.detail || "Action failed";
  ...
  const handleDeleteMessage = async (forEveryone = false) => {
    ...
    try {
      await axios.delete(`${API_BASE_URL}/api/auth/chat/delete-message/`, {
        data: { message_id: selectedMsg.id, for_everyone: forEveryone },
        headers: { Authorization: `Bearer ${token}` },
      });
      setSelectedMsg(null);
      fetchMessages(selectedRoom.id);
  ...
  const handleClearChat = async () => {
    ...
    try {
      await axios.delete(`${API_BASE_URL}/api/auth/chat/delete-message/`, {
        data: { room_id: selectedRoom.id, clear_all: true },
        headers: { Authorization: `Bearer ${token}` },
      });
      setMessages([]);
  ...
  // 🛠️ 100% കൃത്യത ഉറപ്പാക്കിയ ഇൻവിറ്റേഷൻ റിലേഷൻഷിപ്പ് ലോജിക്
  const initiatedById = selectedRoom?.initiated_by && typeof selectedRoom.initiated_by === 'object'
    ? Number(selectedRoom.initiated_by.id)
    : Number(selectedRoom?.initiated_by);

  const isReceivedRequest = Number(selectedRoom?.other_user?.id) === initiatedById;
  const isMeInitiator = initiatedById === currentUserId || (!isReceivedRequest && selectedRoom?.status === "pending");
  const isAdminChat = selectedRoom?.is_admin_chat || selectedRoom?.other_user?.is_admin;
  
  // റിക്വസ്റ്റ് എനിക്ക് വന്നതാണെങ്കിൽ മാത്രം ബട്ടണുകൾ കാണിക്കുക, ഞാൻ അയച്ചതാണെങ്കിൽ വെയിറ്റിങ് മെസ്സേജ് കാണിക്കുക.
  const showRequestActions = selectedRoom?.status === "pending" && !isMeInitiator && !isAdminChat;
        ...
        <div className="chat-rooms-list">
          {chatRooms.length === 0 ? (
            ...
            chatRooms.map((room) => (
              ...
              >
                <img 
                  src={getImageUrl(room.other_user?.image)} 
                  alt="" 
                  onError={(e) => handleImageError(e, getDisplayName(room.other_user))}
                />
          ...
          <>
            <div className="chat-main-header">
              <div className="header-left">
                ...
                />
                <img 
                  src={getImageUrl(selectedRoom.other_user?.image)} 
                  alt="" 
                  onError={(e) => handleImageError(e, getDisplayName(selectedRoom.other_user))}
                />
                <div>
            ...
            >
              {messages.map((msg) => {
                ...
                if (isMe) bubbleClass = "sent";
                else if (isAdmin) bubbleClass = "admin-msg";

                return (
                  <div
                    key={msg.id}
                    className={`message ${bubbleClass} ${selectedMsg?.id === msg.id ? "selected" : ""}`}
            ...
            {showRequestActions ? (
              <div className="chat-request-box">
                ...
                <div className="request-actions">
                  <button onClick={() => handleRequestAction("accept")} className="btn-accept">
                    <FaCheck /> Accept
                  </button>
                  <button onClick={() => handleRequestAction("reject")} className="btn-reject">
                    <FaTimes /> Reject
                  </button>
                  <button onClick={() => handleRequestAction("ignore")} className="btn-ignore">
                    Ignore
                  </button>
                </div>
              </div>
            ) : selectedRoom.status === "pending" && isMeInitiator && !isAdminChat ? (
              <div className="chat-pending">
                Waiting for {getDisplayName(selectedRoom.other_user)} to accept your request...
              </div>
            ) : selectedRoom.status === "rejected" && !isAdminChat ? (
              <div className="chat-rejected">This chat request was rejected.</div>
            ) : isChatAccepted ? (
              <div className="chat-input-container">
                  ...
                  <div className="reply-box">
                    <div className="reply-content">
                      ...
                      <div>
                        <p className="reply-name">
                          Replying to{" "}
                          {replyTo.is_me ? "yourself" : replyTo.sender_name || getDisplayName(selectedRoom.other_user)}
                        </p>
                        <p className="reply-text">{replyTo.text || "📷 Image"}</p>
                      </div>
                    </div>
                ...
                <div className="chat-input">
                  <IoHappy className="input-icon emoji-btn" onClick={() => setShowEmoji(!showEmoji)} />
                  <input
                    type="text"
                    value={newMessage}
                  ...
                  <div className="input-actions">
                    {newMessage.trim() ? (
                      ...
                      <>
                        ...
                        />
                        <IoImage className="image-btn" onClick={() => fileInputRef.current.click()} />
                      </>
                    )}
                  </div>
                </div>
